...
const WEBSIM_API_BASE = 'https://api.websim.com/v1';
...
async function callOllama(model, prompt) {
  try {
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 300000); // 30s
...
async function callWebsim(messages, apiKey) {
    ...
    try {
        ...
        const res = await fetch(`${WEBSIM_API_BASE}/chat/completions`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${key}`
            },
            body: JSON.stringify({ model: 'websim/pro', messages, max_tokens: 2048, temperature: 0.2 }),
            signal: controller.signal
        });
        clearTimeout(timeoutId);
        ...
        if (!res.ok) {
            const txt = await res.text();
            throw new Error(`Websim API error ${res.status}: ${txt}`);
        }
        const j = await res.json();
        return j?.choices?.[0]?.message?.content ?? JSON.stringify(j);

    } catch (error) {
        if (error.name === 'AbortError') throw new Error('Websim API request timed out.');
        throw new Error(`Websim API connection failed: ${error.message}`);
    }
}
